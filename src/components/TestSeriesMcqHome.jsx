...
const TestSeriesMcqHome = () => {
  ...
  return (
    ...
    <header className={`${isDarkMode ? 'bg-gray-800' : 'bg-cyan-700'} p-4 flex justify-between items-center`}>
      <h1 className="text-3xl font-bold text-white">Datasense</h1>
      <div className="flex items-center">
        <button
          onClick={() => setIsDarkMode(!isDarkMode)}
          className={`mr-4 p-2 rounded-full ${isDarkMode ? 'bg-yellow-400 text-gray-900' : 'bg-gray-800 text-yellow-400'}`}
        >
          ...
          {selectedQuizzes.map((quiz) => (
            <div key={quiz._id} className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg shadow-lg overflow-hidden transition-transform duration-300 hover:scale-105`}>
              <div className="p-4 sm:p-6">
                <h2 className="text-xl sm:text-2xl font-bold mb-2">{quiz.quizName}</h2>
                ...
                <p className={`mb-4 text-sm sm:text-base ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  {quiz.description}
                </p>
                <button
      ...
      <footer className={`mt-auto py-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-200'}`}>
        <div className="container mx-auto px-4">
          <div className={`flex items-center justify-center p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-white'} shadow-md`}>
            ...
            <p className={`text-center text-sm sm:text-base ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
