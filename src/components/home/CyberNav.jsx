...
const CyberNav = ({ onLearnSQLClick }) => {
    ...
    <nav className="absolute top-0 left-0 right-0 z-40 pt-2 sm:pt-3 px-3 sm:px-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          >
            <button
              className={`relative w-10 h-10 sm:w-12 sm:h-12 bg-gray-800/50 backdrop-blur-lg rounded-full border border-cyan-400/30 flex items-center justify-center transition-all duration-300 ${showSocial ? 'bg-cyan-400/20 border-cyan-400/60 shadow-lg shadow-cyan-400/25' : 'hover:bg-cyan-400/10'
                }`}
              onTouchStart={handleTouchStart}
            >
              <Bell className={`w-5 h-5 sm:w-6 sm:h-6 text-cyan-400 transition-all duration-300 ${showSocial ? 'animate-pulse scale-110' : ''
                }`} />
